import React, { useEffect, useState } from "react";
import DOMPurify from "dompurify";
import { MdDelete } from "react-icons/md";

import { Container } from "react-bootstrap";
import { useNavigate, useParams } from "react-router-dom";

const userIconURl =
  "data:image/png;base64,";
const ViewEmail = () => {
  const [fetchedEmail, setFetchedEmail] = useState({
    subject: "",
    content: "",
    to: "",
    from: "",
    timestmps: "",
  });
  const { type, emailKey } = useParams();
  const navigate = useNavigate();

  const getEmail = () => {
    let url;
    const { email } = JSON.parse(localStorage.getItem("user"));
    const emailId = email.replace(".", "_");
    if (type === "sendEmail") {
      url = `https://mail-box-c-default-rtdb.firebaseio.com/${emailId}/sent/${emailKey}.json`;
    } else {
      url = `https://mail-box-c-default-rtdb.firebaseio.com/${emailId}/inbox/${emailKey}.json`;
    }
    fetch(url)
      .then((response) => response.json())
      .then((res) => {
        if (res.error) throw new Error(res.error);

        setFetchedEmail(res);
      })
      .catch((err) => {
        console.log(err);
        alert(err.message);
      });
  };

  useEffect(() => {
    getEmail();
  }, [emailKey, type]);

  function formatDateString(dateString) {
    const date = new Date(dateString);

    const monthNames = [
      "Jan",
      "Feb",
      "Mar",
      "Apr",
      "May",
      "Jun",
      "Jul",
      "Aug",
      "Sep",
      "Oct",
      "Nov",
      "Dec",
    ];

    const month = monthNames[date.getMonth()];
    const day = date.getDate();
    const year = date.getFullYear();
    let hours = date.getHours();
    const minutes = date.getMinutes();

    const amOrPm = hours >= 12 ? "PM" : "AM";

    hours = hours % 12 || 12;

    const formattedMinutes = minutes.toString().padStart(2, "0");

    return `${month} ${day}, ${year}, ${hours}:${formattedMinutes} ${amOrPm}`;
  }

  const deleteEmail = () => {
    let url;
    const { email } = JSON.parse(localStorage.getItem("user"));
    const emailId = email.replace(".", "_");
    if (type === "sendEmail") {
      url = `https://mail-box-c-default-rtdb.firebaseio.com/${emailId}/sent/${emailKey}.json`;
    } else {
      url = `https://mail-box-c-default-rtdb.firebaseio.com/${emailId}/inbox/${emailKey}.json`;
    }
    fetch(url, { method: "DELETE" })
      .then((response) => navigate(-1))
      .catch((err) => {
        console.log(err);
        alert(err.message);
      });
  };

  const formatedContent = DOMPurify.sanitize(fetchedEmail.content);
  return (
    <Container className="w-[65vw] md:w-[80vw] h-full mx-auto   p-3 flex flex-col ">
      <h1 className="text-3xl my-2 border-b-2 w-full pb-2 flex justify-between">
        {fetchedEmail.subject}
        <span onClick={deleteEmail}>
          <MdDelete
            size={"20px"}
            className="hover:text-slate-500 rounded-full hover:bg-slate-100"
          />
        </span>
      </h1>
      <div className="flex justify-between border-b-2 mb-3 mt-3  pb-3">
        <div className="flex gap-3  items-center">
          <img
            src={userIconURl}
            className="bg-inherit"
            width={"40px"}
            alt="User icon"
          />
          <div className="flex flex-col">
            <span className="text-[12px]">
              {type === "sendEmail" ? "To." : "From."}
            </span>
            {type === "sendEmail" ? fetchedEmail.to : fetchedEmail.from}
          </div>
        </div>
        <div className="items-center justify-center">
          {formatDateString(fetchedEmail.timestmps)}
        </div>
      </div>
      <div
        id="content"
        dangerouslySetInnerHTML={{ __html: formatedContent }}
      ></div>
    </Container>
  );
};

export default ViewEmail;
